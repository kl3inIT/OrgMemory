...
This plan is intentionally not implementation-active until the Asset Registry
slice of the Spring Modulith package refactor no longer overlaps these files
and the characterization gates below pass.
...
- [x] Record the independent architecture verdict and incorporate every
  ...
  direction.
- [ ] Characterize current create, review, direct Skill publication, role
...
- [ ] Confirm the active Modulith refactor has landed or declared a non-overlap.

## 1. Contract-first product model

- [ ] Split released-consumption/detail DTOs from governance/history DTOs first.
  Write failing tests proving Viewer access never exposes Drafts, review
  comments, unreleased payloads, or role principals.
- [ ] Write failing core and API tests for one owner, direct share, Editor Draft
...
- [ ] Write security-negative tests proving a Viewer cannot edit or read
...
- [ ] Define the Catalog projection for owner, sharing state, current exact
  Release, and Skill activation. Preserve opaque denial behavior.
- [ ] Replace new review/publication affordances with served
  `can_manage_sharing`, `can_transfer_ownership`, `can_publish_direct`, and
  `can_withdraw`; browser controls remain projections only.
- [ ] Update OpenAPI and regenerate the web client only after the core contract
  is executable.

## 2. Compatibility persistence and authorization

- [ ] Add Flyway persistence for typed canonical user, group-member, and
...
- [ ] Extend the authorization outbox with versioned WRITE and DELETE
...
- [ ] Add target OpenFGA permissions while legacy relations remain valid;
...
- [ ] Generalize owner-only direct publication to every enabled Asset profile.
  First Share and owner Save update each validate and create one immutable
  Revision and Release with `DIRECT` provenance.
- [ ] Permit Editors to edit the working Draft only. Keep publication, sharing,
  ownership transfer, and ordinary withdrawal owner-only.
- [ ] Commit first Release, canonical audience intent, and audit evidence in one
...
- [ ] Implement ownerless-only administrator recovery with a locked vacancy
  predicate and audit evidence; add a separate `can_emergency_withdraw` path.
- [ ] Implement asset-level withdrawal of every non-withdrawn Release and
...
- [ ] Add an actor-scoped Enabled preference for visible Skills; specify the
...
- [ ] Filter Assistant Skill discovery by live visibility plus the actor's
...
- [ ] Change primary scopes to **Available to me** and **Created by me**.
...
- [ ] Make Share one bounded modal for people, groups, or organization Viewer.
  ...
  sharing.
- [ ] Replace the primary Governance tabs with a contributor surface centered
...
- [ ] Remove active Review and Deprecated controls. Keep truthful explanation
  that structural validation is not independent review.
- [ ] Keep the current Catalog free of a primary Space taxonomy; this is a
...
- [ ] Supersede decision 0022 rather than editing it. Record why direct
...
- [ ] Reconcile `docs/specs/domains/asset-registry.md` and its test mirror only
...
- [ ] Run focused core/API/OpenFGA/web tests while iterating, then terminating
