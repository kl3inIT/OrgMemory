...
| Onyx | User-file upload has no antivirus, ClamAV, malware, or DLP gate. It checks configured byte/token limits, password protection, extractability, and embedded-image resource caps before storage and async indexing. | `tmp/onyx/backend/onyx/server/features/projects/projects_file_utils.py`, `tmp/onyx/backend/onyx/db/projects.py`, `tmp/onyx/backend/onyx/server/documents/connector.py` | Match the bounded admission mechanics in this increment; malware/DLP is an explicit owner-waived future capability, not a hidden READY state. |
