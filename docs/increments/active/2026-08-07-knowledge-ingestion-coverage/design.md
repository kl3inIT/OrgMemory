...
Five format families become fifteen across sixteen accepted suffixes because
HTML and HTM are one format family with two extensions.
...
- Parsing is a reusable adapter in `integrations:document-parsing-spring-ai`.
  The neutral port and canonical block IR remain in
  `components:graph-rag-core`; Knowledge admission and per-format product limits
  remain in Source Ledger.
- ODS and ODP are included with ODT. The product admits fifteen format families
  through sixteen suffixes: CSV, DOC/DOCX, HTML/HTM, Markdown, ODP, ODS, ODT,
  PDF, PPT/PPTX, RTF, TXT, and XLS/XLSX.
- A DOCX generated by Microsoft Word with built-in Heading 1 produces a typed
  `HEADING`, closing the synthetic-fixture uncertainty.
- `PagePdfDocumentReader` remains the PDF reader so page provenance is
  unchanged. Paragraph/bookmark-aware PDF parsing remains a separate future
  evaluation, not an implicit change in this increment.
