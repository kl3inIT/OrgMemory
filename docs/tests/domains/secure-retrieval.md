...
| Citation opening consumes Source Ledger-owned immutable evidence without revision/blob persistence leakage, preserves typed unavailable audit reasons, and closes integrity-mismatched content with a deny audit before any allow audit | `SourceCitationEvidenceQueryTests`, `CitationContentServiceTests#storageIntegrityMismatchClosesContentAndRecordsDenyAudit`, `ModulithVerificationTests#retrievalDoesNotDependOnSourceLedgerCitationPersistenceOrStatusTypes`, `#citationRepresentationsUseTheSourceLedgerOwnerQuery` |
| Replay hydration returns determinate partial visibility in fixed 20-check batches and fails closed on indeterminate batches | `CanonicalEvidenceAuthorizationServiceTests` partial, batching, and indeterminate scenarios |
| Citation excerpts reauthorize and recheck current evidence, audit the surface, cap Unicode content, and select a closed filename-based presentation kind | `CitationEvidenceServiceTests`, `CitationContentControllerTests`, `CitationContentWebMvcTests` |
