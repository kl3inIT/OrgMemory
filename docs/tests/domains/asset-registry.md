...
| Browser Skill ZIP import preflights file/namespace input, loads live authorized Space targets, retains same-origin multipart CSRF protection, creates a private Draft through the canonical endpoint, and navigates to Governance | `skill-upload-validation.test.ts`, `asset-registry-golden-poc.spec.ts#asset catalog defaults to a grid and keeps list state in the URL` | covered |
